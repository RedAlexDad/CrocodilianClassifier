...
function App() {
  ...
  const items: IProcessedItem[] = FURNITURE_MOCK.map((i) => ({
    ...i,
    score: 0,
    isVisible: true,
  }));
...
self.addEventListener("message", async (event) => {
  ...
  try {
    if (type === "init") {
      ...
      const { pooler_output: textOutput } =
        await SiglipService.textModel(text_inputs);
